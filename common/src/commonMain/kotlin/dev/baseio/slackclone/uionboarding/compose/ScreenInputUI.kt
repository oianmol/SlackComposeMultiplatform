...
fun EmailAddressInputUI(composeNavigator: ComposeNavigator, emailInputVM: EmailInputVM) {
  ...
  CommonInputUI(
    { modifier ->
      ...
      NextButtonWithText(modifier = Modifier, "Next") {
    ...
    when {
      ...
      uiState.workspaces != null -> when {
        uiState.workspaces?.isEmpty() == true -> "No workspace(s) found..."
...
private fun WorkspacesList(
  modifier: Modifier,
  workspacesList: List<DomainLayerWorkspaces.SKWorkspace>,
  workspaceSelected: (DomainLayerWorkspaces.SKWorkspace) -> Unit
) {
  LazyColumn(modifier) {
...
fun WorkspaceInputUI(composeNavigator: ComposeNavigator, workspaceInputVM: WorkspaceInputVM) {
  ...
  CommonInputUI(
    ...
    bottomView = {
      when (uiState) {
        WorkspaceInputVM.UiState.Empty -> NextButtonWithText(modifier = Modifier, "Next") {
          workspaceInputVM.onNextClick()
        }

        is WorkspaceInputVM.UiState.Exception -> NextButtonWithText(modifier = Modifier, "Next") {
          workspaceInputVM.onNextClick()
        }

        WorkspaceInputVM.UiState.Loading -> FloatingActionButton(onClick = {

        }, backgroundColor = Color.White) {
          CircularProgressIndicator(color = SlackCloneColor)
        }

        WorkspaceInputVM.UiState.LoggedIn -> Unit
        is WorkspaceInputVM.UiState.Workspace -> {
          NextButtonWithText(modifier = Modifier, "Login") {
            workspaceInputVM.onLogin()
          }
          NextButtonWithText(modifier = Modifier, "Register") {
            workspaceInputVM.onRegister()
          }
        }
      }
...
fun WorkspaceLoginView(
  modifier: Modifier,
  workspace: DomainLayerWorkspaces.SKWorkspace,
  email: String?,
  password: String?,
  ...
  Column(
    ...
    Text(
      ...
      style = SlackCloneTypography.h6.copy(color = SlackCloneColorProvider.colors.textPrimary)
    )
    Text(
