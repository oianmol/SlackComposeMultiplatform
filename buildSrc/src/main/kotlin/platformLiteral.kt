...
open class PatchedPodGenTask : PodGenTask() {
  ...
  private fun getPodfileContent(specRepos: Collection<String>, xcodeTarget: String) =
    buildString {
      ...
      //}
      pods.get().filterNotNull().mapIndexed { index, it ->
        buildString {
          append("pod '${it.name}'")
  ...
  private fun extracted(name:String,source:PodLocation?=null): String {
    return buildString {
      append("pod '${"$name"}'")

      val pathType = when (source) {
        is Path -> "path"
        is Url -> "path"
        is Git -> "git"
        else -> null
      ...
      val path = source?.getLocalPathAccessor(project, name)
...
fun <T : Any, R> KClass<T>.invokeDynamic(methodStart: String, instance: Any?, vararg params: Any?): R {
  val method =
    this.java.methods.firstOrNull { it.name.startsWith(methodStart) } ?: error("Can't find accessor for $methodStart")
...
fun PodLocation.getLocalPathAccessor(project: Project, podName: String): String? =
  this::class.invokeDynamic("getLocalPath", this, project, podName)
...
private object CocoapodsErrorHandlingUtil {
  ...
  fun handlePodInstallSyntheticError(
    command: String,
    retCode: Int,
    error: String,
    family: Family,
